...
<div>
  ...
  <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="checkDesc" label="检查结果" >         </el-table-column>
  ...
  <el-dialog title="安全检查信息" :visible.sync="dialogFormVisible" width="30%">
    <el-form label-width="80px" size="big">
      ...
      <el-form-item label="检查结果">
        <el-select v-model="form.checkDesc" placeholder="选择" style="width: 100px">
          <el-option
...
export default {
  ...
  methods:{
    ...
    load(){
      this.request.get("/securitycheck/page", {
        params: {
    ...
    save(){
      this.request.post("/securitycheck",this.form).then(res=>{
        if(res) {
    ...
    //*删除数据
    del(id){
      this.request.delete("/securitycheck/"+id).then(res=>{
        if(res){
    ...
    delBatch(){
      ...
      this.request.post("/securitycheck/del/batch",ids).then(res=>{
        if(res){
