...
<div>
  ...
  <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="cleanDate" label="消杀日期" >         </el-table-column>
    <el-table-column prop="useDrugs" label="使用药品">         </el-table-column>
  ...
  <el-dialog title="消杀信息" :visible.sync="dialogFormVisible" width="30%">
...
<!--        <el-input v-model="form.useDrugs" autocomplete="off"></el-input>-->
        <el-select v-model="form.useDrugs" placeholder="选择" style="width: 100px">
          <el-option
...
export default {
  ...
  methods:{
    ...
    load(){
      this.request.get("/cleaninfo/page", {
        params: {
    ...
    save(){
      this.request.post("/cleaninfo",this.form).then(res=>{
        if(res) {
    ...
    //*删除数据
    del(id){
      this.request.delete("/cleaninfo/"+id).then(res=>{
        if(res){
    ...
    delBatch(){
      ...
      this.request.post("/cleaninfo/del/batch",ids).then(res=>{
        if(res){
