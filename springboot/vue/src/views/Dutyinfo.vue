...
<div>
  ...
  <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="dutyDate" label="签到时间" >         </el-table-column>
    <el-table-column prop="planTime" label="值班日期安排" >         </el-table-column>
  ...
  <el-dialog title="值班信息" :visible.sync="dialogFormVisible" width="30%">
    <el-form label-width="120px" size="big">
      ...
      <el-form-item label="值班日期安排">
        <el-date-picker v-model="form.planTime" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss">
        </el-date-picker>
      </el-form-item>
...
export default {
  ...
  methods:{
    ...
    load(){
      this.request.get("/dutyinfo/page", {
        params: {
    ...
    save(){
      this.request.post("/dutyinfo",this.form).then(res=>{
        if(res) {
    ...
    //*删除数据
    del(id){
      this.request.delete("/dutyinfo/"+id).then(res=>{
        if(res){
    ...
    delBatch(){
      ...
      this.request.post("/dutyinfo/del/batch",ids).then(res=>{
        if(res){
